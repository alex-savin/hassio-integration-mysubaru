...
class MySubaruLock(LockEntity):
    ...
    async def async_lock(self, **kwargs) -> None:  # type: ignore[override]
        ...
        if resp.status >= 400:
            ...
            raise HomeAssistantError(f"Lock failed ({resp.status}): {detail or 'unknown error'}")
        await resp.text()
    ...
    async def async_unlock(self, **kwargs) -> None:  # type: ignore[override]
        ...
        if resp.status >= 400:
            detail = await resp.text()
            raise HomeAssistantError(f"Unlock failed ({resp.status}): {detail or 'unknown error'}")
        await resp.text()
